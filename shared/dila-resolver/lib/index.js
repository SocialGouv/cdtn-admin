"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getLatestVersion = exports.extractArticleId = exports.createGetArticleReference = void 0;
function createGetArticleReference(client) {
    return async function getArticle(id) {
        try {
            const apiResult = (await client.fetch({
                method: "POST",
                params: {
                    id,
                },
                path: "/consult/getArticle",
            }));
            const data = checkApiResponse(id, apiResult);
            const [lastVersion] = data.article.articleVersions.slice(-1);
            const containerId = data.article.conteneurs.length > 0
                ? data.article.conteneurs[0].cid
                : data.article.textTitles[0].id;
            return {
                dila_cid: data.article.cid,
                dila_container_id: containerId,
                dila_id: lastVersion.id,
                title: `${data.article.nature} ${data.article.num}`,
                url: "",
            };
        }
        catch (e) {
            console.error(`Error retrieving info about article id ${id}`, e);
            return null;
        }
    };
}
exports.createGetArticleReference = createGetArticleReference;
function getLatestVersion(articleVersions) {
    return articleVersions.reduce((latest, article) => {
        if (parseFloat(article.version) > parseFloat(latest.version)) {
            return article;
        }
        return latest;
    });
}
exports.getLatestVersion = getLatestVersion;
function extractArticleId(url) {
    const match = /(?:LEGI|KALI)ARTI\d{12}/.exec(url);
    return match && match.length > 0 ? match.slice(0, 1) : [];
}
exports.extractArticleId = extractArticleId;
function checkApiResponse(id, data) {
    if (Object.keys(data).length === 1) {
        throw new Error(`invalid response for ${id}, payload: ${JSON.stringify(data)}`);
    }
    return data;
}
